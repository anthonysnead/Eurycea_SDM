...
```{r, response comparision function}
...
```{r, make overlap list function}
make.overlap.list <- function(
  df,
  tree,                          # ape::phylo
  value_col    = "d",            # numeric pairwise value; may be "i" or "d"
  sp1_col      = "Species_1",
  sp2_col      = "Species_2",
  var_col      = "Variable",
  class_col    = "Classification",
  diag_value   = 1,
  agg_fun      = base::mean,     # combine duplicate pairs
  require_full = TRUE,
  verbose      = TRUE
){
  ## --- checks
  need <- c(sp1_col, sp2_col, var_col, class_col, value_col)
  miss <- setdiff(need, names(df))
  if (length(miss)) stop("Input df is missing: ", paste(miss, collapse = ", "))
  if (!inherits(tree, "phylo")) stop("'tree' must be an ape::phylo object.")

  ## --- ensure numeric 'value_col' (handles factor/char)
  x <- df[[value_col]]
  ...
  bad <- which(is.na(x_num) & !is.na(df[[value_col]]))
  if (length(bad)) {
    stop("Column '", value_col, "' contains non-numeric values. Examples: ",
         paste(utils::head(unique(df[[value_col]][bad]), 5), collapse = ", "))
  }
  df[[value_col]] <- x_num

  ## --- clean & restrict to tree species
  df <- df[!is.na(df[[class_col]]) & !is.na(df[[value_col]]), , drop = FALSE]
  df[[sp1_col]] <- as.character(df[[sp1_col]])
  df[[sp2_col]] <- as.character(df[[sp2_col]])

  species <- sort(tree$tip.label)
  df <- df[df[[sp1_col]] %in% species & df[[sp2_col]] %in% species, , drop = FALSE]
  if (nrow(df) == 0L) stop("No rows remain after filtering to tree species.")

  ## --- collapse duplicates by unordered pair within each Variable
  #  Use .i/.j to avoid name collisions with value_col (e.g., "i")
  i_min <- pmin(df[[sp1_col]], df[[sp2_col]])
  j_max <- pmax(df[[sp1_col]], df[[sp2_col]])
  agg <- stats::aggregate(
    df[[value_col]],
    by = list(Variable = df[[var_col]], .i = i_min, .j = j_max),
    FUN = agg_fun
  )
  names(agg)[4] <- value_col  # keep your numeric column with its real name

  vars <- sort(unique(agg$Variable))
  n    <- length(species)

  if (isTRUE(verbose)) {
    cat("\n[make.overlap.list]\n")
    cat("Species:", n, " | Variables:", length(vars), "\n")
  }

  ## --- helper to build one symmetric block (numeric-safe)
  build_block <- function(v){
    sub <- agg[agg$Variable == v, , drop = FALSE]

    M <- matrix(NA_real_, n, n, dimnames = list(species, species))
    storage.mode(M) <- "double"

    ii <- match(sub$.i, species)
    jj <- match(sub$.j, species)
    if (anyNA(ii) || anyNA(jj)) {
      badsp <- unique(c(sub$.i[is.na(ii)], sub$.j[is.na(jj)]))
      stop("Variable '", v, "' references species not in tree: ",
           paste(badsp, collapse = ", "))
    }

    vals <- as.numeric(sub[[value_col]])  # <- now guaranteed not to collide
    M[cbind(ii, jj)] <- vals

    Mt <- t(M)
    copy_from_t <- is.na(M)  & !is.na(Mt)
    copy_to_t   <- !is.na(M) &  is.na(Mt)
    both        <- !is.na(M) &  !is.na(Mt)

    M[copy_from_t] <- Mt[copy_from_t]
    Mt[copy_to_t]  <- M[copy_to_t]
    if (any(both)) M[both] <- (M[both] + Mt[both]) / 2

    diag(M) <- as.numeric(diag_value)

    if (isTRUE(require_full) && anyNA(M)) {
      miss <- which(is.na(M) & upper.tri(M), arr.ind = TRUE)
      ex <- if (nrow(miss)) paste0(rownames(M)[miss[1,1]], "–", colnames(M)[miss[1,2]]) else "unknown"
      stop("Variable '", v, "' has missing pair(s) after symmetrizing (e.g., ", ex,
           "). Set require_full = FALSE to allow NAs.")
    }
    M
  }

  OL <- setNames(lapply(vars, build_block), vars)

  ## --- attach Variable -> Category map
  vm <- unique(df[, c(var_col, class_col)])
  class_map <- stats::setNames(vm[[class_col]], vm[[var_col]])
  attr(OL, "Classification") <- class_map

  if (isTRUE(verbose)) {
    cats <- unname(class_map[vars])
    cat("Categories:", paste(unique(cats), collapse = ", "), "\n")
    nas <- vapply(OL, function(M) anyNA(M), logical(1))
    if (any(nas)) cat("Matrices with NA:", paste(names(OL)[nas], collapse = ", "), "\n")
  }
...
```{r, kernel pcoa}
run.kernel.pgls <- function(
  OL,                   # named list of species x species matrices (per Variable)
  tree,                 # ape::phylo
  kmax      = 50,
  k_mode    = c("cumvar","grid"),
  cumvar    = 0.90,
  k_grid    = NULL,
  iter      = 9999,
  seed      = 123,
  pairwise  = TRUE,
  verbose   = TRUE,
  .global_Y_name = ".Ytmp_kernel_rrpp"  # global LHS (matrix) name
){
  k_mode <- match.arg(k_mode)

  ## deps
  for (pkg in c("Matrix","ape","RRPP")) {
    if (!requireNamespace(pkg, quietly = TRUE))
      stop("Package '", pkg, "' is required.")
  }

  ## inputs / mapping checks
  if (!inherits(tree, "phylo")) stop("'tree' must be an ape::phylo object.")
  class_map <- attr(OL, "Classification")
  if (is.null(class_map)) stop("attr(OL,'Classification') is missing.")
  if (anyNA(class_map))  stop("Classification map contains NA values.")

  species <- sort(tree$tip.label)
  vars_ol <- names(OL)
  if (is.null(vars_ol) || !length(vars_ol)) stop("OL must be a named list with variables as names.")
  missing_in_map <- setdiff(vars_ol, names(class_map))
  if (length(missing_in_map))
    stop("Variables missing from Classification map: ", paste(missing_in_map, collapse = ", "))

  ## observation IDs & phylo Cov
  row_ids <- unlist(lapply(vars_ol, function(v) paste(v, species, sep="|")))
  P <- ape::vcv(tree, corr = TRUE)
  if (!all(species %in% rownames(P))) stop("Some tree tips missing from ape::vcv rownames.")
  Cov.big <- as.matrix(Matrix::bdiag(
    replicate(length(vars_ol), P[species, species, drop=FALSE], simplify = FALSE)
  ))
  dimnames(Cov.big) <- list(row_ids, row_ids)

  ## per-variable blocks (symmetrize, diag=1, same order)
  prep_block <- function(M, sp){
    M <- as.matrix(M)
    M <- M[sp, sp, drop = FALSE]
    M <- (M + t(M))/2
    diag(M) <- 1
    M
  }
  K_blocks <- lapply(vars_ol, function(v) prep_block(OL[[v]], species))
  names(K_blocks) <- vars_ol

  ## big kernel + global double-centering
  K  <- as.matrix(Matrix::bdiag(K_blocks))
  N  <- length(row_ids)
  ...
  pos <- which(ev$values > 0)
  if (length(pos) < 2L) stop("Insufficient positive eigenvalues after centering.")
  vals.pos <- ev$values[pos]
  scores.full <- ev$vectors[, pos, drop = FALSE] %*%
    diag(sqrt(vals.pos), nrow = length(vals.pos))
  colnames(scores.full) <- paste0("PC", seq_len(ncol(scores.full)))
  rownames(scores.full) <- row_ids

  cum   <- cumsum(vals.pos) / sum(vals.pos)
  k_cap <- min(kmax, length(pos))
  if (k_mode == "cumvar") {
    k_cand <- which(cum >= cumvar)[1]; if (is.na(k_cand)) k_cand <- length(cum)
    k <- min(k_cand, k_cap)
    k_table <- data.frame(k = seq_len(k_cap), cumvar = cum[seq_len(k_cap)])
  } else {
    if (is.null(k_grid)) k_grid <- seq_len(k_cap)
    k_grid <- k_grid[k_grid >= 1 & k_grid <= k_cap]
    if (!length(k_grid)) stop("k_grid has no valid values within 1..", k_cap)
    k <- max(k_grid)
    k_table <- data.frame(k = k_grid, cumvar = cum[k_grid])
  }
  if (isTRUE(verbose)) {
    cat(sprintf("\n--- k selection ---\nk.cap: %d \nk chosen:%d  (cumvar=%.4f)\n",
                k_cap, k, cum[k]))
  }

  ## DESIGN: predictors only in data.frame; response kept separate
  Z_all <- data.frame(
    Species  = factor(rep(species, times = length(vars_ol)), levels = species),
    Variable = factor(rep(vars_ol,  each  = length(species)), levels = vars_ol),
    row.names   = row_ids,
    check.names = FALSE
  )
  Z_all$Category <- factor(
    unname(class_map[as.character(Z_all$Variable)]),
    levels = unique(unname(class_map[vars_ol]))
  )
  if (anyNA(Z_all$Category)) {
    bad <- unique(as.character(Z_all$Variable[is.na(Z_all$Category)]))
    stop("NA in Category; missing mapping for variables: ", paste(bad, collapse = ", "))
  }

  ## Response matrix (multivariate) – assign to GLOBAL to mimic interactive behavior
  pcs_keep <- paste0("PC", seq_len(k))
  Ymat <- as.matrix(scores.full[, pcs_keep, drop = FALSE])
  stopifnot(identical(rownames(Ymat), rownames(Z_all)))
  assign(.global_Y_name, Ymat, envir = .GlobalEnv)
  on.exit(try(remove(list = .global_Y_name, envir = .GlobalEnv), silent = TRUE), add = TRUE)

  ## Build RHS-only formula and precompute model frame
  fm_rhs <- stats::as.formula("~ Category + Species")
  mf     <- stats::model.frame(fm_rhs, data = Z_all, na.action = stats::na.fail)
  kept   <- rownames(mf)

  ## Subset predictors, Cov, and the GLOBAL Y to the exact same rows
  Z_sub    <- Z_all[kept, , drop = FALSE]
  Cov_sub  <- Cov.big[kept, kept, drop = FALSE]
  Y_global <- get(.global_Y_name, envir = .GlobalEnv)
  Y_sub    <- Y_global[kept, , drop = FALSE]
  # reassign the subset back to global so LHS matches rows used by RRPP
  assign(.global_Y_name, Y_sub, envir = .GlobalEnv)

  ## FINAL identity check
  if (!identical(rownames(Z_sub), rownames(Cov_sub)) ||
      !identical(rownames(Z_sub), rownames(Y_sub))) {
    stop("Rowname mismatch after alignment.\n",
         "ex in data not in Cov: ",
         paste(utils::head(setdiff(rownames(Z_sub), rownames(Cov_sub)), 5), collapse = ", "),
         "\nex in Cov not in data: ",
         paste(utils::head(setdiff(rownames(Cov_sub), rownames(Z_sub)), 5), collapse = ", "),
         "\nex in Y not in data: ",
         paste(utils::head(setdiff(rownames(Y_sub), rownames(Z_sub)), 5), collapse = ", "))
  }

  ## Fit with matrix LHS in GLOBAL (exactly like your working interactive run)
  options(contrasts = c("contr.sum","contr.poly"))
  set.seed(seed)
  fm_full <- stats::as.formula(paste0(.global_Y_name, " ~ Category + Species"))
  ...
  aov_tab <- anova(fit, effect.type = "F")

  ## Pairwise (optional)
  pw_sum <- NULL
  if (isTRUE(pairwise)) {
    pw_sum <- try({
      pw <- RRPP::pairwise(fit, groups = model.frame(fit)$Category)
      summary(pw, confidence = 0.95, test.type = "dist")
    }, silent = TRUE)
    if (inherits(pw_sum, "try-error")) pw_sum <- NULL
  }

  if (isTRUE(verbose)) {
    cat("\n--- ANOVA (Category row) ---\n")
    print(aov_tab$table[rownames(aov_tab$table) == "Category", , drop = FALSE])
  }
  ...
  list(
    inputs = list(
      ...
      k_mode    = k_mode,
      cumvar    = cumvar,
      k_grid    = if (k_mode == "grid") k_grid else NULL,
      iter      = iter,
      seed      = seed
    ),
    kernel = list(
      K   = K,
      Kc  = Kc,
      eig = list(values.pos = vals.pos, cumvar = cum, k_cap = k_cap),
      scores = scores.full
    ),
    data = list(Z_all = Z_all, Z_used = Z_sub),
    cov  = list(P = P, Cov.big = Cov.big, Cov.used = Cov_sub),
    k_selection = list(k = k, table = k_table),
    fit  = list(rrpp = fit, anova = aov_tab, pairwise = pw_sum)
  )
}
```
...
aggregate(d ~ Classification, data = Unadjusted.df, FUN = mean)

aggregate(i ~ Classification, data = Unadjusted.df, FUN = mean)
```


```{r, overlap list }
Compare.Responses.Unadjust.D.List <- make.overlap.list(
  df = (Compare.Responses.Unadjust %>%
    dplyr::filter(!is.na(Classification))),
  tree = Binded.Tree,
  value_col = "d",
  require_full = TRUE,   # set FALSE if some variables lack complete species coverage
  verbose = TRUE
)
```

```{r, model fit undajusted D}
Unadjusted.D.res <- run.kernel.pgls(
  OL       = Compare.Responses.Unadjust.D.List,
  tree     = Binded.Tree,
  kmax     = 50,
  k_mode   = "cumvar",
  cumvar   = 0.90,
...
```{r, overlap list I }
Compare.Responses.Unadjust.I.List <- make.overlap.list(
  df =  (Compare.Responses.Unadjust %>%
    dplyr::filter(!is.na(Classification))),
  tree = Binded.Tree,
  value_col = "i",
...
```{r, modelf fit undajusted D}
Unadjusted.I.res <- run.kernel.pgls(
  OL       = Compare.Responses.Unadjust.I.List,
  tree     = Binded.Tree,
  kmax     = 50,
  k_mode   = "cumvar",
  cumvar   = 0.90,
...
aggregate(d ~ Classification, data = Adjusted.df, FUN = mean)

aggregate(i ~ Classification, data = Adjusted.df, FUN = mean)

```


```{r, overlap list }
Compare.Responses.Adjust.D.List <- make.overlap.list(
  df = (Compare.Responses.Adjust %>%
    dplyr::filter(!is.na(Classification))),
  tree = Binded.Tree,
  value_col = "d",
  require_full = TRUE,   # set FALSE if some variables lack complete species coverage
...
```{r, modelf fit undajusted D}
Adjusted.D.res <- run.kernel.pgls(
  OL       = Compare.Responses.Adjust.D.List,
  tree     = Binded.Tree,
  kmax     = 50,
  k_mode   = "cumvar",
  cumvar   = 0.90,
...
```{r, overlap list I }
Compare.Responses.Adjust.I.List <- make.overlap.list(
  df =  (Compare.Responses.Adjust %>%
    dplyr::filter(!is.na(Classification))),
  tree = Binded.Tree,
  value_col = "i",
...
```{r, modelf fit undajusted D}
Adjusted.I.res <- run.kernel.pgls(
  OL       = Compare.Responses.Adjust.I.List,
  tree     = Binded.Tree,
  kmax     = 50,
  k_mode   = "cumvar",
  cumvar   = 0.90,
...
Eco.Var.pglmm.df <- na.omit(Eco.Var.Importance$Importance.Model.df) %>%
  dplyr::mutate(Classification = factor(Classification, levels = c("Urbanization",
                                                                   "Temperature",
                                                                   "Precipitation")),
                Species = as.factor(Species))

Eco.Var.pglmm.df$Classification <- relevel(
  factor(Eco.Var.pglmm.df$Classification, 
         levels = c("Urbanization", "Temperature", "Precipitation")),
  ref = "Urbanization"
)

contrasts(Eco.Var.pglmm.df$Classification) <- contr.treatment(3)

Phylo.cor <- ape::vcv(Binded.Tree, corr = TRUE)

Eco.Var.pglmm.res <- pglmm(
  Importance ~ Classification + (1 | Species__),
  data      = Eco.Var.pglmm.df,
  cov_ranef = list(Species = Phylo.cor),
  family    = "gaussian",
  REML      = TRUE
)

summary(Eco.Var.pglmm.res)

Lm.equiv <- lm(Importance ~ Classification, data = Eco.Var.pglmm.df)
emm.ecovar <- emmeans(Lm.equiv, ~ Classification)
pairs(emm.ecovar)

aggregate(Importance ~ Classification, Eco.Var.pglmm.df, mean)
```
...
Plot.Occs <- Occs.Rare %>%
 # dplyr::filter(Longitude != min(Longitude)) %>%
  ...
  dplyr::mutate(Clade = dplyr::case_when(Species %in% c("Eurycea multiplicata", 
                                                        "Eurycea tynerensis") ~ paste0("Clade ", as.character(as.roman(1))),
                                         Species %in% c("Eurycea bislineata",
                                                        "Eurycea aquatica",
                                                        "Eurycea wilderae",
                                                        "Eurycea cirrigera") ~ paste0("Clade ", as.character(as.roman(2))),
                                         Species %in% c("Eurycea lucifuga",
                                                        "Eurycea guttolineata",
                                                        "Eurycea longicauda") ~ paste0("Clade ", as.character(as.roman(3))),
                                         Species %in% c("Eurycea hillisi",
                                                        "Eurycea quadridigitata",
                                                        "Eurycea chamberlaini",
                                                        "Eurycea paludicola") ~ paste0("Clade ", as.character(as.roman(4)))),
                Clade = factor(Clade, levels = c("Clade I", "Clade II", "Clade III", "Clade IV")))
...
Plot.Occs.sf <- st_as_sf(Plot.Occs, coords = c("Longitude", "Latitude"), crs = 4326) %>%
  group_by(Species, Clade) %>%  # Group by Clade too
  summarise(geometry = st_convex_hull(st_union(geometry)), .groups = "drop")  # Keep Clade
...
Colors <- c("#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", "#8c564b", 
            "#e377c2", "#7f7f7f", "#bcbd22", "aquamarine3", "darkgoldenrod2", "#17becf", 
            "#c6c6c6")
...
ggplot() +
  ...
  annotation_north_arrow(location = "tl", which_north = "true", 
    ...
    text_size = 6  # <-- Adjust this value to control the size of "N"
  )) +
